<template>
  <div>
    <v-btn text @click="show = !show" block>
      <v-icon>{{ show ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon> คำขอ
      <v-spacer></v-spacer>
    </v-btn>
    <v-expansion-panels v-show="show">
      <v-expansion-panel v-for="(item,i) in req" :key="i" :class="reqClass(item.type)">
        <v-expansion-panel-header disable-icon-rotate>
          <template v-slot:actions>
            <div style="display: flex; flex-wrap: wrap; flex-direction: row-reverse;">
              <v-btn
                @click="managed(item, true)"
                color="success" dark> อนุญาต</v-btn>
              <div style="width: 1vw;" />
              <v-btn
                @click="managed(item, false)"
                color="error" outlined> ปฎิเสธ</v-btn>
              <div style="width: 1vw;" />
              <v-btn icon>
        ...
        <v-expansion-panel-content style="color: gray; font-size: 80%;">
          <v-text-field label="เหตุผลของผู้ขอ" v-model="item.reason" readonly dense></v-text-field>
          <v-text-field label="เหตุผล" v-model="item.confirmedReason" dense></v-text-field>
          <v-icon small>mdi-clock</v-icon>ส่งเมื่อ: {{ formatDate(item.requestTime) }}
